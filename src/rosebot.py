...
class DriveSystem(object):
    ...
    def go_straight_for_inches_using_encoder(self, inches, speed):
        ...
        pos0 = self.left_motor.get_position()
        self.go(speed, speed)
        while True:
            pos = self.left_motor.get_position()
            if abs(pos - pos0) >= inches * 360 / (1.3 * math.pi):
                self.stop()
                break
    ...
    def go_straight_until_color_is(self, color, speed):
        ...
        print('start')
        while True:
            if self.sensor_system.color_sensor.get_color_as_name() == color or self.sensor_system.color_sensor.get_color() == color:
                print('stop')
                self.stop()
                break
    ...
    def display_camera_data(self):
        ...
        (if any).
        """
        blob_data = self.sensor_system.camera.get_biggest_blob()
        print(blob_data)
...
class ArmAndClaw(object):
    ...
    def calibrate_arm(self):
        ...
        print('position reset, should be approximately 0: ', self.motor.get_position())
    ...
    def move_arm_to_position(self, desired_arm_position):
        ...
        while True:
            ...
            if pos1 <= goal + 5 and pos1 >= goal -5:
                self.motor.turn_off()
                break
            if self.touch_sensor.is_pressed():
                self.motor.turn_off()
